...
for (i in 1:n_simulations) {
  ...
  ######################
  ## Pairwise t-tests ##
  pairwise_tukey_result <- pairwise.t.test(df$Degeneration, df$Dosage, p.adj = "none")
  
  if (any(na.omit(pairwise_tukey_result$p.value) < alpha)) {
...
type_ii_error_analysis <- function(rho, n_simulations = 100, alpha = 0.05) {
  ...
  for (i in 1:n_simulations) {
    ...
    group_low <- data[data[, 1] == 0.50, ]
    mwu_result1 <- wilcox.test(group_high[,1], group_middle[,2])
    mwu_result2 <- wilcox.test(group_high[,2], group_low[,2])
    mwu_result3 <- wilcox.test(group_low[,1], group_middle[,2])
    alpha_adjusted <- alpha / 3
    # Check if it's a Type II error (p-value is less than alpha)
    if (mwu_result2$p.value < alpha){
  ...
  results <- data.frame(
    ...
    type_ii_rate__tukey=power_tukey,
    type_ii_rate__anova=power_anova,
    power_mwu = power_mwu,
...
print(results_df)
#write.csv(results_df, "type_ii_error_final_thesis.csv")
```
